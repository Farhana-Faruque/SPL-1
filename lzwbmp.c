...
unsigned char* lzw_compress(unsigned char* input, unsigned int input_size, unsigned int* output_size) {
    ...
    while (input_pos < input_size) {
        unsigned char next_char = input[input_pos++];
...
unsigned char* lzw_decompress(unsigned char* input, unsigned int input_size, unsigned int original_size) {
    ...
    unsigned short old_code = input[input_pos++] | (input[input_pos++] << 8);
    output[output_pos++] = (unsigned char)old_code;

    while (input_pos < input_size) {
        unsigned short new_code = input[input_pos++] | (input[input_pos++] << 8);
        unsigned char* temp = malloc(original_size); // Temporary buffer for string
        unsigned int temp_pos = 0;

        if (new_code >= dict_size) {
            temp[temp_pos++] = dict[old_code].append;
            unsigned short temp_code = old_code;
            while (temp_code != 0xFFFF) {
                temp[temp_pos++] = dict[temp_code].append;
                temp_code = dict[temp_code].prefix;
            }
        } else {
            unsigned short temp_code = new_code;
            while (temp_code != 0xFFFF) {
                temp[temp_pos++] = dict[temp_code].append;
                temp_code = dict[temp_code].prefix;
            }
...
int compressBMP2(const char* input_file, const char* output_file) {
    ...
    fwrite(&dataSize, sizeof(unsigned int), 1, fout); // Store original size
...
int decompressBMP2(const char* input_file, const char* output_file) {
    ...
    if (fread(&original_size, sizeof(unsigned int), 1, fin) != 1) {
